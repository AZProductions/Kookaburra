namespace KookaburraShell
{
    class custom_commands //old code.
    {
        public static string name1 { get; set; }
        public static string loc1 { get; set; }
        public static string name2 { get; set; }
        public static string loc2 { get; set; }
        public static string name3 { get; set; }
        public static string loc3 { get; set; }
        public static string name4 { get; set; }
        public static string loc4 { get; set; }
        public static string name5 { get; set; }
        public static string loc5 { get; set; }
        public static string name6 { get; set; }
        public static string loc6 { get; set; }
        public static string name7 { get; set; }
        public static string loc7 { get; set; }
        public static string name8 { get; set; }
        public static string loc8 { get; set; }
        public static int num { get; set; }
    }
}
